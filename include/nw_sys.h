...
namespace yf
{
    namespace sys
    {
        ...
        public:
            ...
            void DoTasksTest(const int& last_job_id, const int& cur_job_id, const int& task_group_id);
        ...
        private:
            /// Phase2: Point Cloud: Get TMat logistic
            void GetTMatLogistic(std::deque<yf::data::arm::MissionConfig>& arm_mission_configs, int& n, const int& task_group_id);
...
//        DoTasksTest(last_job_id, cur_job_id, nw_status_ptr_->db_cur_task_group_id);
...
void yf::sys::nw_sys::GetTMatLogistic(std::deque<yf::data::arm::MissionConfig>& arm_mission_configs, int& n, const int& task_group_id)
{
    auto arm_mission_config_dir = "../data/point_clouds/real/arm_mission_config_" + std::to_string(arm_mission_configs[n].id);
    std::filesystem::create_directory(arm_mission_config_dir);
...
void yf::sys::nw_sys::DoTasksTest(const int &last_job_id, const int &cur_job_id, const int &task_group_id)
{
    /// 0. Initialization --- flow control
    //
    nw_status_ptr_->cur_job_success_flag = false;
                    ...
                    mir100_ptr_->GetPLCRegisterIntValue(4) != 3)
            {
                ...
                if(ugv_mission_continue_flag)
                {
                    ...
                    if(arm_wait_plc003_success_flag)
                    {
                        ...
                        int  cur_arm_config_id = sql_ptr_->GetArmConfigId(cur_model_config_id_, cur_order);
                        int  is_valid = sql_ptr_->GetArmConfigIsValid(cur_arm_config_id);
                        int  is_motion_valid = sql_ptr_->GetArmConfigIsMotionValid(cur_arm_config_id);
                        ...
                        switch(is_valid)
                        {
                            ...
                            case 1: // valid
                            {
                                ...
                                if(arm_wait_plc001_success_flag)
                                {
                                    ...
                                    // is cur_order belonging to valid order? of course

                                    auto current_tool = tm5.GetCurrentTool();

                                    if(is_motion_valid == true && current_tool == data::arm::Tool::None)
                                    {
                                        switch (cur_task_mode_)
                                        {
                                            case data::arm::TaskMode::UVCScanning:
