...
namespace yf
{
    namespace sql2
    {
        ...
        public:
            ...
            void UpdateScheduleRecordData(const int& schedule_id, const int& schedule_status);

            void UpdateDeviceTable(const std::string& device_name, const int& connection_status, const int& mission_status);
            // Job
            //
            // each schedule -operation == each schedules
            // each job --- each schedule
            // each task --- each task
            //
            void RetrieveAllJobs(const int& schedule_id);
        ...
        private:
            ...
            std::string database_ = "NW_mobile_robot_system";
            std::string Uid_ = "sa";
...
yf::sql2::sql_server::sql_server(const std::string &driver, const std::string &server, const std::string &database,
                                const std::string &Uid, const std::string &Pwd)
...
void yf::sql2::sql_server::setODBCConnectionStr(const std::string &driver, const std::string &server, const std::string &database,
                                               const std::string &Uid, const std::string &Pwd)
{
    ODBCConnectionStr_ = "Driver={"+driver+"};Server="+server+";Database="+database+";Uid="+Uid+";Pwd="+Pwd;
}

std::string yf::sql2::sql_server::getODBCConnectionStr()
{
    return ODBCConnectionStr_;
}

bool yf::sql2::sql_server::isConnected()
{
    return conn_.connected();
}

void yf::sql2::sql_server::Connect()
{
    try
    {
...
void yf::sql2::sql_server::Disconnect()
{
    try
    {
        conn_.disconnect();
...
std::string yf::sql2::sql_server::TimeNow()
{
    struct tm newtime;
    char am_pm[] = "AM";
...
void yf::sql2::sql_server::UpdateScheduleData(const int& schedule_id, const int& schedule_status)
{
    ...
    try
    {
        ...
        switch (schedule_status)
        {
            ...
            {
                query_update = "UPDATE schedule_table SET status = " + status_str + ", actual_start='"+TimeNow()+"' WHERE ID = "+ id_str;
                break;
            }

            case 3: // finish
            {
                query_update = "UPDATE schedule_table SET status = " + status_str + ", actual_end='"+TimeNow()+"' WHERE ID = "+ id_str;
                break;
            }
...
void yf::sql2::sql_server::UpdateScheduleRecordData(const int &schedule_id, const int &schedule_status)
{
    std::string status_str = std::to_string(schedule_status);
    ...
    try
    {
        ...
        switch (schedule_status)
        {
            case 2: // in processing
            {
                query_update = "INSERT INTO schedule_record_table(schedule_id, actual_start, status) VALUES (" +
            ...
            case 3: // finish
            {
                query_update = "UPDATE schedule_record_table SET status = " + status_str + ", actual_end='" + TimeNow() + "' WHERE schedule_id = " + schedule_id_str + " AND status=2";
                break;
            }
...
void yf::sql2::sql_server::RetrieveAllAvailableScheduleId()
{
    std::string query_update;
    ...
    try
    {
        ...
        query_update = "SELECT ID FROM schedule_table where status=1 AND planned_start > '"+ TimeNow() +"'";
...
void yf::sql2::sql_server::WaitAvailableSchedules()
{
...
std::deque<int> yf::sql2::sql_server::GetSchedulesId() {
...
std::deque<int> yf::sql2::sql_server::GetJobsId(const int& cur_schedule_id)
{
    std::string query_update;
    ...
    try
    {
        ...
        query_update = "SELECT job_id FROM schedule_job_table where schedule_id = " + schedule_id_str + " ORDER BY job_order" ;
...
std::deque<int> yf::sql2::sql_server::GetTasksId(const int& cur_job_id)
{
    std::string query_update;
    ...
    try
    {
        ...
        query_update = "SELECT ID FROM schedule_job_task_table where job_id = " + job_id_str + " ORDER BY task_order" ;
...
void yf::sql2::sql_server::RetrieveAllJobs(const int &schedule_id)
{

}

void yf::sql2::sql_server::GetScheduleExecTime(const int &id)
{
    std::string query_update;
    ...
    try
    {
        ...
        query_update = "SELECT planned_start FROM schedule_table where ID = "+std::to_string(id)+" AND status=1 AND planned_start > '"+ TimeNow() +"'";
...
std::string yf::sql2::sql_server::get_execute_time()
{
    return schedule_id_execute_time;
}

void yf::sql2::sql_server::UpdateDeviceTable(const std::string &device_name, const int &connection_status,
                                            const int &mission_status)
{
    ...
    try
    {
        ...
        query_update = "UPDATE device_table SET connection_status = " + connection_status_str + ", mission_status = " + mission_status_str + ", modified_date='" + TimeNow() + "' WHERE device = '"+device_name+"'" ;
...
std::vector<float> yf::sql2::sql_server::Time_str2vector(std::string str)
{
...
bool yf::sql2::sql_server::isLatestSchedule(std::string time_from_database, std::string time_now)
{
...
void yf::sql2::sql_server::WaitForExecute(const std::string &execute_time)
{
