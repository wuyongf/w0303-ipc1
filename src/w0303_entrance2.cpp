...
public:
    ...
    void GetStatusLog()
    {
        ...
        if(arm_connection_status_ == yf::common::ConnectionStatus::Connected)
        {
        ...
        {
            case yf::common::MissionStatus::Error:
            {
                arm_mission_status_str = "Error";
                break;
            }
            case yf::common::MissionStatus::Running:
            {
                arm_mission_status_str = "Running";
                break;
            }
            case yf::common::MissionStatus::Idle:
            {
                arm_mission_status_str = "Idle";
                break;
            }
            case yf::common::MissionStatus::Finish:
            {
                arm_mission_status_str = "Finish";
                break;
            }
            case yf::common::MissionStatus::Pause:
            {
...
public:
    ...
    void StartupIPCServer()
    {
        if(ipc_port_ == 12345)
        {
            ...
            // Yes. Update the connection status now.
    ...
    void CloseIPCServer()
    {
        ...
        //
        // (1) Arm
        arm_connection_status_ = yf::common::ConnectionStatus::Disconnected;
        arm_mission_status_ = yf::common::MissionStatus::Error;
    ...
    void CloseArmLnClient()
    {
        ...
        std::unique_lock<std::mutex> ul(mux_ln_Blocking);
        cv_ln_Blocking.notify_one();
    ...
    {
        while(arm_mission_status_ != yf::common::MissionStatus::Finish)
        {
            // wait 500ms.
    ...
    // Arm --- Status
    yf::common::MissionStatus GetArmStatus()
    {
        return arm_mission_status_;
    }
...
//        GetStatusLog();
    ...
    void CheckArmInitMssionStaus()
    {
        ...
        while (check_flag)
        {
            ...
            switch (arm_mission_status_)
            {
                case yf::common::MissionStatus::Idle:
                {
                    // todo: Everything is fine. The arm is waiting any mission to do. Enjoy.
                    arm_mission_flag_ = true;

                    // todo:
                    //  1. update the current Job, task status?  Idle ---> Running

                    //  2. no need to check again, break
                    check_flag = false;
                    break;
                }
                ...
                case yf::common::MissionStatus::Error:
                {
                    // todo: The Arm is Error!
                    arm_mission_flag_ = false;

                    // todo:
                    //  1. update the current Job, task status?  ---> Error....
                ...
                case yf::common::MissionStatus::Pause:
                {
                    // todo: The Arm has been paused!

                    // todo:
                    //  1. update the current Job, task status?  ---> Error....

                    //  2. wait 2s and then break, keep checking
                ...
                {
                    // todo:
                    //  1. just keep checking for now....
                    std::cout << "From check funtion1, the arm is running.... " << std::endl;
                ...
                {
                    // TODO:
                    // todo: The Arm has Finished!
                    arm_mission_flag_ = false;

                    // todo:
                    //  1. update the current Job, task status?  ---> Running and Finish....

                    //  2. need to check again, wait 2s and then recheck
                    std::this_thread::sleep_for(std::chrono::milliseconds(2000));
                    break;
                }
            }
    ...
    void AssignArmMission(const std::string& arm_command_str)
    {
        ...
        if(arm_mission_flag_)
        {
            ...
            if(arm_command_str == "EnterListenNode")
            {
                ...
                arm_ln_connection_status_ = yf::common::ConnectionStatus::Connected;
                ...
                arm_ln_connection_status_ = yf::common::ConnectionStatus::Disconnected;
    ...
    void WaitSchedules()
    {
       while(schedule_flag)
       {
           while(wait_schedule_flag)
           {
               ...
               while(arm_mission_status_ != yf::common::MissionStatus::Idle)
               {
       ...
       LOG(INFO) << "schedule flag is false, thread: wait schedules shut down...";

    };

    // Assign Each Task
    //
    void AssignTasks(yf::manager::TaskForRefModel& task)
    {
        // For each task
        //
        task.Clear();
        ...
        //
        task.model_config.name =  yf::models::ModelName::Desk;
        ...
        {
            yf::arm::MotionConfig arm_motion_config;

            arm_motion_config.clean_type = yf::arm::CleanType::Mopping;
            arm_motion_config.ref_operation_area = yf::arm::OperationArea::Right;
            ...
            arm_motion_config.arm_motion_mode = yf::arm::MotionMode::NetCommand;
    ...
    void AssignJobs()
    {
        ...
        if(time_flag)
        {
            ...
            {
                // Assign Job
                yf::manager::Job schedule;

                schedule.appointment_config.location = yf::manager::Location::HKSTP;
                schedule.appointment_config.floor = 2;
                schedule.appointment_config.start_time = "2021-01-28 18:02:00.000";
                schedule.task_no = 4;

                schedule.tasks.clear();

                for(int j =0 ; j < schedule.task_no ; j++)
                {
                    yf::manager::TaskForRefModel task;
    ...
    void DoSchedule()
    {
        ...
        while(schedule_flag)
        {
            ...
            while (!do_schedule_flag)
            {
                //wait for 2s.
                std::cout << "wait do schedule flag = true..." << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(2000));
            }
            ...
            {
                auto schedule = schedules.front();
                schedules.pop_front();

                // wait for execute time...
                //
                auto time_now = schedule.currentDateTime();
                auto time_temp = time_now;
                auto time_scheduled = schedule.appointment_config.start_time;
                ...
                if(IsLatestSchedule(time_scheduled, time_now))
                {
                    ...
                    {
                        time_temp = schedule.currentDateTime();
                        std::this_thread::sleep_for(std::chrono::milliseconds(2000));
                    }

                    // TODO: Execute the schedule!
                    DoTask(schedule.tasks);
            ...
            // No more schedules
            //
            // reset do_schedule_flag. wait for trigger by wait schedule
    ...
    void DoTask(std::deque<yf::manager::TaskForRefModel> tasks)
    {
        while (!tasks.empty())
        {
    ...
    void set_schedule_status(const yf::common::MissionStatus& status)
    {
...
private:
    ...
    std::mutex mux_ln_Blocking;
    std::condition_variable cv_ln_Blocking;

    // Job
    //
    //
    yf::common::MissionStatus schedule_status_;
    ...
    std::deque<yf::manager::Job> schedules{};
